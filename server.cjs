...
async function handleAiAssistant(req, res) {
  ...
  const systemPrompt = [
    ...
    'For historical route-chart questions, use traffic.routeTravelHistory. These are summarized TranStar Historical Travel Time chart samples from previous completed NRG match dates. Use sampleDates, dateSummaries, preGame, postGame, congestionWindows, peakTravelMin, typicalTravelMin, and noDataDates to estimate likely travel time and congestion periods before and after a game. State the sample coverage when giving estimates.',
    ...
    'If asked about Travel Chart history, use traffic.routeTravelHistory when it has sampleDates > 0. If it is loading, say historical chart summaries are still loading. If a route has no sample dates, say the historical chart returned no samples for that route/date set. Do not invent values beyond routeTravelHistory.',
